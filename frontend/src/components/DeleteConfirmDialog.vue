  ...
  <div v-if="modelValue" class="fixed inset-0 z-[100] flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full mx-4" style="animation: fade-in 0.2s ease-out, slide-up 0.2s ease-out;">
      ...
      <div class="flex items-center gap-3 mb-4">
        <div :class="['w-10 h-10 rounded-full flex items-center justify-center', isPrimary ? 'bg-zinc-100' : 'bg-red-50']">
          <LogOut v-if="isPrimary" class="w-5 h-5 text-zinc-500" />
          <AlertTriangle v-else class="w-5 h-5 text-red-500" />
        </div>
        <div>
          <h3 class="text-lg font-semibold text-zinc-900">{{ title }}</h3>
          <span :class="['inline-block px-2 py-0.5 rounded-full text-xs font-medium mt-0.5', isPrimary ? 'bg-zinc-100 text-zinc-600' : 'bg-red-50 text-red-600']">{{ badgeText }}</span>
        </div>
      </div>
      ...
      <div class="flex gap-3">
        <button @click="closeDialog" :disabled="loading" class="flex-1 px-4 py-2.5 border border-zinc-200 text-zinc-900 rounded-xl text-sm font-medium hover:bg-zinc-50 transition-colors disabled:opacity-50">
          {{ cancelText }}
        </button>
        <button @click="handleConfirm" :disabled="loading" :class="['flex-1 px-4 py-2.5 text-white rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-70', isPrimary ? 'bg-zinc-900 hover:bg-zinc-800' : 'bg-red-500 hover:bg-red-600']">
